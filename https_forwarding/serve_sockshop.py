...
def load(path):
	try:
		resp = requests.request(method=request.method, url=request.url.replace(request.host_url, 'http://10.0.0.2/'), headers={key: value for (key, value) in request.headers if key != 'Host'}, data=request.get_data(), cookies=request.cookies, timeout=20)
		...
		for header in headers:
			modified_header =  (header[0] , header[1].replace('10.0.0.2',args.domain+':'+str(args.port)).replace('; HttpOnly',''))
			modified_headers.append(modified_header)
		response = Response(resp.content.replace('https://10.0.0.2', 'https://'+args.domain+':'+str(args.port)).replace('http://10.0.0.2','https://'+args.domain+':'+str(args.port)).replace('10.0.0.2',args.domain+':'+str(args.port)), resp.status_code , modified_headers)
